<?php
    ...
    if (!$isloggedin) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            $current_title = htmlspecialchars($row['title']);
            $current_content = htmlspecialchars($row['content']);
            $current_image = htmlspecialchars($row['image_path']);
        } else {
            echo "<script>alert('Discussion not found');window.location.href='your_discussions.php';</script>";
            exit();
        }
    }
?>
			...
			<div id="wrapper">
				...
				<!-- Main -->
                    ...
                    <div class="edit-discussion-form">
                        <form method="post" action="edit_discussion.php?id=<?=$_GET['id']?>" enctype="multipart/form-data">
                            <div class="field half first">
                                <label for="title">Title</label>
                                <input type="text" name="title" id="title" placeholder="<?php echo $current_title ?>">
                            </div>
                            <br>
                            ...
                            <div class="field half first">
                                ...
                                <input type="text" name="content" id="content" placeholder="<?php echo $current_content?>">
                            </div>
                            <br>
                        ...
                        if ($_SERVER["REQUEST_METHOD"] == "POST") {
                            ...
                            try {
                                if (empty($target_file)) {
                                    mysqli_query($conn, "UPDATE discussions SET title='$title', content='$content' WHERE id={$_GET['id']}");
                                }
                                else {
                                    mysqli_query($conn, "UPDATE discussions SET title='$title', content='$content' , image_path='$target_file' WHERE id={$_GET['id']}");
                                }
